...
public struct Configuration {
    ...
    public static var `default`: Configuration {
        return Configuration(
            ...
            capturePattern: #"(?:.*(\".*\").localized)|NSLocalizedString\(\s*"([^"]+)"(?:\s*,|\s*comment:\s*"[^"]*"\s*\))|IDLocalizedString\(\s*"([^"]+)""#
        )
    }
    
    //#"(?:.*(\".*\").localized)|NSLocalizedString\(\s*"([^"]+)"(?:\s*,|\s*comment:\s*"[^"]*"\s*\))"#

    private init(localizableEncoding: String.Encoding, fileEncoding: String.Encoding, formatsSupported: [FormatsSupported], capturePattern: String) {
